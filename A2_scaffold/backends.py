...
SCRIPTS = {
    ...
    {
        "thought":
            "I only have the referral id. Fetch the referral first.",
        "calls": [
            (
                "get_referral",
    ...
    {
        "thought":
            "The referral is for OPH. I need to check the specialty "
            "criteria and patient history. These calls are independent, "
            "so I can run them in the same turn.",
        "calls": [
            (
...

    {
        "final": {
            ...
            "reason":
                "Ophthalmology requires VF-01. "
                "The referral contains IOP-03, which does not satisfy "
                "the mandatory visual field test requirement. "
                "No slot search was performed because the referral "
                "is incomplete.",
        },
        "thought":
            "VF-01 is missing, so according to the routing rule "
            "I must request the specific missing test and stop. "
            "There is no reason to search clinic slots.",
    },
],
"REF-5590": [
    {
        "thought":
            "I only have the referral id, so I must fetch the referral.",
        "calls": [
            (
                "get_referral",
    ...
    {
        "thought":
            "The referral is for Ophthalmology. I need to check "
            "the referral criteria. I can also retrieve patient history "
            "independently in the same turn.",
        "calls": [
            (
                "check_referral_criteria",
                {
...

    {
        "final": {
            ...
            "reason":
                "The clinical summary contains the Ophthalmology "
                "red-flag term 'sudden visual loss'. "
                "The referral must be escalated to the triage nurse. "
                "VF-01 is present, but completion of mandatory tests "
                "does not override a red flag. No clinic slot was booked.",
        },
        "thought":
            "A red flag outranks all later checks. "
            "Stop immediately rather than querying clinic slots.",
    },
],
...
class LiveBackend:
    ...
    Records the ACTUAL usage returned by the API for each model call.
    """
    ...
    def __init__(
        ...
        # Usage from the MOST RECENT model call.
    ...
    def token_estimate(self, transcript):
        """Return measured tokens from the previous API call.

        The function name is kept because agent.py already calls it.
        For the live backend these are measurements, not estimates.
        """
...
def _live_call(messages):
    """The only vendor-aware function.

    Returns:
        content: model response text
        usage: actual API token counts
    """
